...
### Exercise: main workflow (10 min) {.blauw}
...
![](./images/recipe_dev.png){ width=70%}
...
**2. make a change. in this case: add a file.**
 ...
 - add a file to your repo called "recipe.txt", containing your favorite recipe for chocolate milk. You can either create this file on github, or create it on your laptop and upload it:
 
![](./images/create_file2.png){ width=100%}

![](./images/recept.png){ width=100%}

 - click `commit new file` : 

![](./images/commitrecipe.png){ width=100%}
...
.gitignore files (note the dot) are text files containing a list of files in a repo directory you do not want Git to keep version control on. They will not be uploaded to the github repitory.
 ...
 This needs to be done before asking git to follow a file (staged and commited). A file already added to the index of the repository (i.e. which files git will follow) will not be unfollowed based on the .gitignore. Unfollowing a file requires that you do some coding to remove it from the index:
...
### Demo: distributed workflow

We will demo what a distributed git/github workflow would look like from an R user perspective. If you are in fact an R user, <mark>you can find a tutorial here. </mark>
